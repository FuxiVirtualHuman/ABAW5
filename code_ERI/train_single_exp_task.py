...
from data.dataset_chanllege4 import Dataset_ERI, collate_fn
...
def train_ERI(epoch, loader, net, optimizer, best_Exp_score, net3=None):
    ...
    time0 = time.time()
    for i, data in pbar:
        imgs, labels, audio, text, lengths = data['imgs'], data['labels'], data['audio'], data['text'], data['lengths']
        imgs, labels, audio, text = imgs.cuda(), labels.cuda(), audio.cuda(), text.cuda() 
        time_data = time.time() - time0
        optimizer.zero_grad()

        pred = net(imgs, text, audio, lengths)
        # Exp_loss
        ...
        time0 = time.time()      
        # pbar.set_description(f'[Train epoch {epoch}(lr:{lr:.5f})]\t loss:{loss.item():.4f}({avg_loss:.4f}) time_data:{time_data:.1f} time_train:{time_train:.1f}' )
...
def test_ERI(epoch, loader, net, best_acc, patience_cur, save_step=10, patience=10):
    ...
    print(f'*{timestamp} [Test epoch {epoch}]\t loss_avg: {avg_loss:.4f}\t pcc:{pcc_avg:.4f}({pcc})\n')
    logger.append(f'*{timestamp} [Test epoch {epoch}]\t loss_avg: {avg_loss:.4f}\t pcc:{pcc_avg:.4f}({pcc})\n')
    with open(os.path.join(log_save_path, f'{timestamp}.log'), "a+") as log_file:
        log_file.writelines(logger)
...
if __name__ == '__main__':
    ...
    train_dataset = Dataset_ERI(data_root, 'train', img_max=img_max, transforms=transform)
    # train_dataset = torch.utils.data.distributed.DistributedSampler(train_dataset)
    train_dataloader = DataLoader(dataset=train_dataset,batch_size=bz,shuffle=True,num_workers=20,collate_fn=collate_fn, drop_last=True)
    
     
    val_dataset = Dataset_ERI(data_root, 'val', img_max=img_max, transforms=transform)
    # val_dataset = torch.utils.data.distributed.DistributedSampler(val_dataset)

    val_dataloader = DataLoader(dataset=val_dataset,batch_size=bz,shuffle=False,num_workers=20,collate_fn=collate_fn, drop_last=True)

    
    
    # model
    net = EffMulti_modality2()
    ...
    optimizer = optim.SGD(filter(lambda p: p.requires_grad, net.parameters()), lr=lr, momentum=0.9,
                          weight_decay=1e-4)
    scheduler = lr_scheduler.CosineAnnealingWarmRestarts(optimizer, 5, 2, 1e-5)
    scaler = GradScaler()
