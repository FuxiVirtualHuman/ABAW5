...
class RepVGGplus(nn.Module):
    ...
    def __init__(self, num_blocks, num_classes, 
                 width_multiplier,in_channels=3, override_groups_map=None,
                 deploy=False,
                 use_post_se=False,
                 use_checkpoint=False, activation=None):
        super().__init__()
        ...
        self.stage0 = RepVGGplusBlock(in_channels=in_channels, out_channels=self.in_planes, kernel_size=3, stride=2, padding=1, deploy=self.deploy, use_post_se=use_post_se)
...
class RepVGGplus_mine(nn.Module):
...
def create_RepVGGplus_L2pse(deploy=False, use_checkpoint=False, num_classes=1000, activation=None,in_channels=3):
    return RepVGGplus(num_blocks=[8, 14, 24, 1], num_classes=num_classes, in_channels=in_channels,
...
def create_RepVGGplus_mine(deploy=False, use_checkpoint=False, num_classes=1000, activation=None,in_channels=3):
    return RepVGGplus_mine(num_blocks=[4, 7, 1], num_classes=num_classes, in_channels=in_channels,
                  width_multiplier=[2.5, 2.5, 5], override_groups_map=None, deploy=deploy, use_post_se=True,
                      use_checkpoint=use_checkpoint, activation=activation)
    
repvggplus_func_dict = {
'RepVGGplus-L2pse': create_RepVGGplus_L2pse,
'RepVGGplus-mine': create_RepVGGplus_mine,
}
...
if __name__ == '__main__':
    ...
    model = bulid_repvgg(deploy=False, num_classes=7, activation='tanh', in_channels=1).cuda()
    inputs = torch.randn((8,1,32,768)).cuda()

    out = model(inputs)['main']
    print(out)