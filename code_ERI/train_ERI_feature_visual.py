...
def train_ERI(epoch, loader, net, optimizer, best_Exp_score, net3=None):
    ...
    for i, data in pbar:
        ...
        pred, fea, labels_mixup = net(imgs, audios, lengths, None, labels, attention_map=attention_map,inputs_t=text,
        ...
        loss = loss_l1
        if out_dim >= 14:
            labels_class = torch.argmax(labels[:,:7], axis=1)
            loss_ce = criterion_ce(pred[:,7:14], labels_class)
            loss +=  loss_ce*weight_ce
            
        if use_triplets:
            loss += criterion_triplet(fea, labels[:,:7]) * 0.04
        
        if use_ccc:
            loss += torch.mean(torch.stack([CCC_Loss(pred[e, :7], labels[e][:7]) for e in range(labels.shape[0])]))
        
        if out_dim >=15:
            loss_country = criterion_bce(pred[:,-1], labels[:,-1])
            loss += loss_country
        
        if use_distill:
            with torch.no_grad():
                pred_teacher, fea_teacher, labels_mixup_teacher = net_teacher(imgs, audios, lengths, pretrained_feature, labels, attention_map=attention_map,inputs_t=text,
                                            return_feature=use_triplets, use_shift=use_shift, use_mixup=use_mixup, )
...
def test_ERI(epoch, loader, net, best_acc, best_pcc, patience_cur, save_step=10, patience=10, save_res=False):
    ...
    for i, data in pbar:
        ...
        # print(imgs)
        with torch.no_grad():
            pred, fea, labels_mixup = net(imgs, audios, lengths, None, attention_map=attention_map,
                                          return_feature=use_triplets, use_shift=use_shift, inputs_t=text)
        # Exp_loss
        # loss_l1 = criterion_l1(pred, labels)
        loss_l1 = criterion_regression(pred[:, :7], labels[:,:7])
        # loss_l1 = criterion_bmc(pred[:,:7], labels)
        ...
        if use_triplets:
            loss += criterion_triplet(fea, labels) * 0.04
...
if __name__ == '__main__':
    ...
    train_dataloader = DataLoader(dataset=train_dataset,batch_size=bz,shuffle=True,num_workers=8,
                                  collate_fn=collate_fn2, drop_last=True, pin_memory=True)
    ...
    val_dataloader = DataLoader(dataset=val_dataset,batch_size=bz,shuffle=False,num_workers=8,
                                collate_fn=collate_fn2, drop_last=True, pin_memory=False)
    ...
    # optimizer = Lion(filter(lambda p: p.requires_grad, net.parameters()), lr=lr/3)
    ...
    if use_mea:
        ...
        model_wa = Vallina_fusion_visual(in_dim=768, out_dim=out_dim, backbone_visual=backbone_visual, use_audio=use_audio, use_text=use_text,
                                use_attention=use_attention, use_dual=use_dual).cuda()
